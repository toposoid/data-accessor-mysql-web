...
trait DocumentAnalysisResultStatesTable {
  ...
  implicit def GetResultDocumentAnalysisResultStatesRow(implicit e0: GR[Long], e1: GR[String], e2: GR[LocalDateTime]): GR[DocumentAnalysisResultStatesRow] = GR{
    prs => import prs._
    (DocumentAnalysisResultStatesRow.apply _).tupled((<<[Long], <<[String], <<[LocalDateTime], <<[LocalDateTime]))
  }
  ...
  class DocumentAnalysisResultStates(_tableTag: Tag) extends profile.api.Table[DocumentAnalysisResultStatesRow](_tableTag, Some("toposoiddb"), "document_analysis_result_states") {
    def * = (id, name, createdAt, updatedAt).<>((DocumentAnalysisResultStatesRow.apply _).tupled, DocumentAnalysisResultStatesRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(name), Rep.Some(createdAt), Rep.Some(updatedAt))).shaped.<>({r=>import r._; _1.map(_=> (DocumentAnalysisResultStatesRow.apply _).tupled((_1.get, _2.get, _3.get, _4.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
